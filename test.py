...
import random
from namemoviebefore import findmovie,response
...
def movie(event):
    ...
    if movie !='':
		...
		with graph.as_default():
			...
			elif (name == '') and (movie_name != '') and classify != 9:  # คำถาม
				Type(classify, event, movie_name, userid, user, question, name)
...
def general(question, event,userid,user):
    if ('ใคร' in question) and (len(question)<15):
        doc = user.find_one({'question':'ใคร'})
        for i in doc['question']:
            if question in i:
                text =random.choice(doc['answer'])
